...
import './HeroSection.css'; // 👈 Add this line (you’ll create it below)
...
const HeroSection = () => {
  ...
  useEffect(() => {
    const timers = [
      setTimeout(() => setVisible(v => ({ ...v, heading: true })), 200),
      setTimeout(() => setVisible(v => ({ ...v, subheading: true })), 600),
  ...
  return (
    ...
    <section className='scroll-mt-16 min-h-screen text-white bg-black flex items-center'>
      ...
      <div className='px-4 md:px-6 mx-auto text-center tracking-tighter'>
        ...
        {/* Tag */}
        {/* <div className={`inline-flex items-center gap-2 px-4 py-2 bg-emerald-500/10 backdrop-blur-sm border border-emerald-500/20 rounded-full mb-8 ${visible.heading ? 'slide-up' : 'hidden-state'
          }`}>
          <Sparkles className="w-4 h-4 text-green-300" />
          <span className="text-sm text-green-400 font-medium">Open To Work</span>
        </div> */}
        <div
          className={`inline-flex items-center gap-2 px-4 py-2 bg-emerald-500/10 backdrop-blur-sm border border-emerald-500/20 rounded-full mb-8 ${visible.heading ? 'slide-up' : 'hidden-state'
            }`}
        >
          {/* Pulsing Dot */}
        ...
        {/* Heading */}
        <h1 className={`text-6xl md:text-8xl ${visible.heading ? 'slide-up' : 'hidden-state'}`}>
          Transform Ideas Into <br /> Digital Success
        </h1>

        {/* Subheading */}
        <h3 className={`text-xl md:text-2xl font-extralight mt-8 text-gray-300 ${visible.subheading ? 'slide-up' : 'hidden-state'}`}>
          We're your partner in product design, website creation, <br />
          and branding for every stage of your business.
        </h3>

        {/* Buttons */}
        <div className={`space-x-5 mt-8 ${visible.buttons ? 'slide-up' : 'hidden-state'}`}>
          <button className='bg-white text-black border px-6 py-2 rounded-lg hover:bg-transparent hover:text-white hover:border-gray-500 transition-all duration-500'>
            Get Started
          </button>
          <button className="text-white px-6 py-2 rounded-lg border border-gray-500 hover:text-black hover:bg-white hover:border-none transition-all duration-500">
